...
function buildNamespace(busline, event) {
    ...
    if (busline !== 'main') {
        if (!exportedNamespace[busline]) {
            ...
            exportedNamespace[busline].destroy = buildDestroy(busline);
        }
        exportedNamespace = exportedNamespace[busline];
    } else {
        mainBusLineRegisteredFunctions.push(emitionFunctionName, resolveFunctionName);
    }
...
function buildDestroy(busline) {
    return function() {
        if (busline === 'main') {
            observerBusLines[busline] = [];
...
exports.destroy = buildDestroy('main');
