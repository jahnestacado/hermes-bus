...
HermesBus.prototype.unsubscribe =  function unsubscribe() {
    ...
    if(observerBusLines[buslineName] && subscribedObject){
        ...
        var isBuslineEmpty = Object.keys(busline).reduce(function(isEmpty, key){
            return isEmpty || !busline[key].length;
        }, false);

        if(buslineName !== DEFAULT_BUSLINE && isBuslineEmpty){
            _utilityBelt.destroyBusLine(self, buslineName);
        }
    }
};
...
var _utilityBelt = {
    ...
    getSubscribedObjectInfo: function getSubscribedObjectInfo(args){
        ...
        if(args.length === 2 && _.isObject(args[1])){
            ...
            throw Exception("hermes-bus.subscribe(): Invalid arguments", args);
        }
