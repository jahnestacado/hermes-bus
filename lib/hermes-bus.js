...
function getEventHandler(busline, event) {
    return function() {
        ...
        args.push(function() {
            console.log("WARN - 'resolve' callback is undefined on triggered event '" + event + "'. Try bus.resolve" + firstLetterToUpperCase(event) + "(...)");
        });
...
function buildNamespace(busline, event) {
    ...
    var triggerFunctionName = "trigger" + firstLetterToUpperCase(event);
    var resolveFunctionName = "resolve" + firstLetterToUpperCase(event);
    if (busline !== 'main') {
        if (!exportedNamespace[busline]) {
            exportedNamespace[busline] = buildBuslineUtils(busline);
        }
        exportedNamespace = exportedNamespace[busline];
    } else {
        mainBusLineRegisteredFunctions.push(triggerFunctionName, resolveFunctionName);
    }

    exportedNamespace[resolveFunctionName] = getPromiseEventHandler(busline, event);
    exportedNamespace[triggerFunctionName] = getEventHandler(busline, event);

}

function buildBuslineUtils(busline) {
    var utils = {
        activate: buildStatusSwitch(busline, true),
        deactivate: buildStatusSwitch(busline, false),
...
exports.on = on;
exports.require = busRequire;
exports.reset = reset;
exports.hardReset = hardReset;
exports.activate = buildStatusSwitch('main', true);
exports.deactivate = buildStatusSwitch("main", false);